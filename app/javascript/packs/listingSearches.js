...
titleSearch.addEventListener("keyup", function (e) {
  filterListingsByTitle(this, e);
});
...
function sortCards(param, cards) {
  switch (param) {
    case "cheapest":
      return cards.sort(
        (cardOne, cardTwo) => cardOne.dataset.price - cardTwo.dataset.price
      );
    case "expensive":
      return cards.sort(
        (cardOne, cardTwo) => cardTwo.dataset.price - cardOne.dataset.price
      );
    case "newest":
      return cards.sort(
        (cardOne, cardTwo) => cardTwo.dataset.created - cardOne.dataset.created
      );
    case "oldest":
      return cards.sort(
        (cardOne, cardTwo) => cardOne.dataset.created - cardTwo.dataset.created
      );
  }
}
  ...
  select.addEventListener("change", () => {
    ...
    cards.forEach((card) => {
      ...
      if (
        ...
        card.dataset.condition.toLowerCase() != conditionSelected.toLowerCase()
      ) {
